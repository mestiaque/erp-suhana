...
<title>{{websiteTitle('Branch/Factory List')}}</title>
@endsection @push('css')
    ...
    <div class="card-body">
        ...
        <form action="{{route('admin.branchs')}}">
            <div class="row">
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{route('admin.branchs')}}">All ({{$total->total}})</a></li>
                        <li><a href="{{route('admin.branchs',['status'=>'active'])}}">Active ({{$total->active}})</a></li>
                        <li><a href="{{route('admin.branchs',['status'=>'inactive'])}}">Inactive ({{$total->inactive}})</a></li>
                    </ul>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <span style="margin:0 5px;">{{$branchs->currentpage()==1?$i+1:$i+($branchs->perpage()*($branchs->currentpage() - 1))+1}}</span>
...
<!-- Add Modal -->
 <div class="modal fade text-left" id="AddDepartment" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
	 ...
	 <form action="{{route('admin.branchsAction','create')}}" method="post">
	   	  @csrf
    	   <div class="modal-header">
    		 <h4 class="modal-title">Add Branch/Factory</h4>
...
<!--Edit Modal -->
@foreach($branchs as $i=>$dpm)
 <div class="modal fade text-left" id="EditDepartment_{{$dpm->id}}" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
	 <div class="modal-content">
	 <form action="{{route('admin.branchsAction',['update',$dpm->id])}}" method="post">
	   	  @csrf
    	   <div class="modal-header">
    		 <h4 class="modal-title">Edit Branch/Factory</h4>
    		 <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    		   <span aria-hidden="true">&times; </span>
    		 </button>
    	   </div>
    	   <div class="modal-body">
    			 ...
    			 <div class="form-group">
    				<label for="name">Address</label>
					<textarea name="address" class="form-control {{$errors->has('address')?'error':''}}" placeholder="Enter address">{!!$dpm->description!!}</textarea>
					@if ($errors->has('address'))
					<p style="color: red; margin: 0; font-size: 10px;">{{ $errors->first('address') }}</p>
					@endif
             	</div>
