...
@section('title')
<title>{{ websiteTitle('Colors List') }}</title>
@endsection
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="dropdown">
            @can('dev.all')
            <a href="javascript:void(0)" class="btn-custom primary" data-toggle="modal" data-target="#Colors" style="padding:5px 15px;">
                <i class="bx bx-plus"></i> Add Color
            </a>
            @endcan
            <a href="{{ route('admin.colors') }}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.colors') }}">
            <div class="row">
                <div class="col-md-12 mb-2">
                    <div class="input-group">
                        <input type="text" name="search" value="{{ request()->search ?? '' }}" placeholder="Color Name" class="form-control {{ $errors->has('search') ? 'error' : '' }}" />
        ...
        <form method="GET" action="{{ route('admin.colors') }}">
            <div class="row mb-2">
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{ route('admin.colors') }}">All ({{ $report->total }})</a></li>
                        <li><a href="{{ route('admin.colors', ['status' => 'active']) }}">Active ({{ $report->active }})</a></li>
                        <li><a href="{{ route('admin.colors', ['status' => 'inactive']) }}">Inactive ({{ $report->inactive }})</a></li>
                    </ul>
                </div>
            </div>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td> <span class="px-4" style="background: {{ $d->short_description }}"></span> &nbsp; {{ $d->name }}</td>
                            <td>{!! $d->description ?? '--' !!}</td>
                            ...
                            <td class="text-center">
                                ...
                                @endcan
                                @can('dev.all')
                                <a href="{{ route('admin.colorsAction', ['delete', $d->id]) }}" class="btn-custom danger" onclick="return confirm('Are You Sure To Delete?')">
...
<!-- Add Modal -->
<div class="modal fade text-left" id="Colors" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.colorsAction', ['add']) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
                        <input type="text" class="form-control {{ $errors->has('name') ? 'error' : '' }}" name="name" placeholder="Enter Color Name" required>
                    </div>
                    <div class="form-group mb-2">
                        <label>Color</label>
...
<div class="modal fade text-left" id="Edit_{{ $d->id }}" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.colorsAction', ['update', $d->id]) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
                    ...
                    <div class="form-group mb-2">
                        <label>Color</label>
                        <input type="color" class="form-control form-control-color" id="colorInput" name="color" value="{{ $d->short_description }}" title="Choose your color">
                    </div>
                    <div class="form-group mb-2">
                        <label>Description</label>
