...
@section('title')
<title>{{ websiteTitle('SKUs List') }}</title>
@endsection
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="dropdown">
            @can('dev.all')
            <a href="javascript:void(0)" class="btn-custom primary" data-toggle="modal" data-target="#SKUs" style="padding:5px 15px;">
                <i class="bx bx-plus"></i> Add SKU
            </a>
            @endcan
            <a href="{{ route('admin.skus') }}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.skus') }}">
            <div class="row">
                <div class="col-md-12 mb-2">
                    <div class="input-group">
                        <input type="text" name="search" value="{{ request()->search ?? '' }}" placeholder="SKU Name" class="form-control {{ $errors->has('search') ? 'error' : '' }}" />
        ...
        <form method="GET" action="{{ route('admin.skus') }}">
            <div class="row mb-2">
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{ route('admin.skus') }}">All ({{ $report->total }})</a></li>
                        <li><a href="{{ route('admin.skus', ['status' => 'active']) }}">Active ({{ $report->active }})</a></li>
                        <li><a href="{{ route('admin.skus', ['status' => 'inactive']) }}">Inactive ({{ $report->inactive }})</a></li>
                    </ul>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                @endcan
                                @can('dev.all')
                                <a href="{{ route('admin.skusAction', ['delete', $d->id]) }}" class="btn-custom danger" onclick="return confirm('Are You Sure To Delete?')">
...
<!-- Add Modal -->
<div class="modal fade text-left" id="SKUs" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add New SKU</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.skusAction', ['add']) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
                        <input type="text" class="form-control {{ $errors->has('name') ? 'error' : '' }}" name="name" placeholder="Enter SKU Name" required>
                    </div>
                    <div class="form-group mb-2">
                        <label>Description</label>
                        <textarea class="form-control" name="description" placeholder="Enter SKU Description"></textarea>
                    </div>
...
<div class="modal fade text-left" id="Edit_{{ $d->id }}" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.skusAction', ['update', $d->id]) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Nmae *</label>
