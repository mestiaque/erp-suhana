...
@section('title')
<title>{{ websiteTitle('Units List') }}</title>
@endsection
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="dropdown">
            @can('dev.all')
            <a href="javascript:void(0)" class="btn-custom primary" data-toggle="modal" data-target="#Units" style="padding:5px 15px;">
                <i class="bx bx-plus"></i> Add Unit
            </a>
            @endcan
            <a href="{{ route('admin.units') }}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.units') }}">
            <div class="row">
                <div class="col-md-12 mb-2">
                    <div class="input-group">
                        <input type="text" name="search" value="{{ request()->search ?? '' }}" placeholder="Unit Name" class="form-control {{ $errors->has('search') ? 'error' : '' }}" />
        ...
        <form method="GET" action="{{ route('admin.units') }}">
            <div class="row mb-2">
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{ route('admin.units') }}">All ({{ $report->total }})</a></li>
                        <li><a href="{{ route('admin.units', ['status' => 'active']) }}">Active ({{ $report->active }})</a></li>
                        <li><a href="{{ route('admin.units', ['status' => 'inactive']) }}">Inactive ({{ $report->inactive }})</a></li>
                    </ul>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                @endcan
                                @can('dev.all')
                                <a href="{{ route('admin.unitsAction', ['delete', $d->id]) }}" class="btn-custom danger" onclick="return confirm('Are You Sure To Delete?')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.unitsAction', ['add']) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
                        <input type="text" class="form-control {{ $errors->has('name') ? 'error' : '' }}" name="name" placeholder="Enter Unit Name" required>
                    </div>
                    <div class="form-group mb-2">
...
<div class="modal fade text-left" id="Edit_{{ $d->id }}" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('dev.all')
                <form action="{{ route('admin.unitsAction', ['update', $d->id]) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
