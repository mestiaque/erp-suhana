...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3>Realization No: {{ $record->realization_no }}</h3>
        <div>
            <a href="{{ route('admin.commercial.realizationAction', ['edit', $record->id]) }}" class="btn btn-success"><i class="bx bx-edit"></i> Edit</a>
            <a href="{{ route('admin.commercial.realization') }}" class="btn btn-secondary">Back</a>
        </div>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                ...
                <table class="table table-borderless">
                    <tr><th width="150">Realization No</th><td>{{ $record->realization_no }}</td></tr>
                    <tr><th>Submission Date</th><td>{{ $record->submission_date ? \Carbon\Carbon::parse($record->submission_date)->format('d M Y') : '' }}</td></tr>
            ...
            <div class="col-md-6">
                ...
                <table class="table table-borderless">
                    ...
                    <tr><th>Realized Value</th><td>{{ $record->currency }} {{ number_format($record->realized_value, 2) }}</td></tr>
                    <tr><th>Net Realized</th><td>{{ $record->currency }} {{ number_format($record->net_realized, 2) }}</td></tr>
                    ...
                    <tr><th>Bank Branch</th><td>{{ $record->bank_branch }}</td></tr>
                </table>
            </div>
        </div>
        @if($record->remarks)
