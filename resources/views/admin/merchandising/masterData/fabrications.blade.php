...
@section('title')
<title>{{ websiteTitle('Fabrications List') }}</title>
@endsection
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3>Fabrications List</h3>
        <div class="dropdown">
            @can('fabrications.add')
            <a href="javascript:void(0)" class="btn-custom primary" data-toggle="modal" data-target="#Fabrications" style="padding:5px 15px;">
                <i class="bx bx-plus"></i> Add Fabrication
            </a>
            @endcan
            <a href="{{ route('admin.fabrications') }}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.fabrications') }}">
            <div class="row">
        ...
        <form method="GET" action="{{ route('admin.fabrications') }}">
            <div class="row mb-2">
                <div class="col-md-4">
                    @if(can('fabrications.edit') || can('fabrications.delete'))
                    <div class="input-group mb-1">
                        <select class="form-control form-control-sm rounded-0" name="action" required>
                            <option value="">Select Action</option>
                            @can('fabrications.edit')
                            <option value="1">Active</option>
                            <option value="2">Inactive</option>
                            @endcan
                            @can('fabrications.delete')
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{ route('admin.fabrications') }}">All ({{ $report->total }})</a></li>
                        <li><a href="{{ route('admin.fabrications', ['status' => 'active']) }}">Active ({{ $report->active }})</a></li>
                        <li><a href="{{ route('admin.fabrications', ['status' => 'inactive']) }}">Inactive ({{ $report->inactive }})</a></li>
                    </ul>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if(can('fabrications.edit') || can('fabrications.delete'))
                                    ...
                                    @endcan
                                    @can('fabrications.delete')
                                    <a href="{{ route('admin.fabricationsAction', ['delete', $d->id]) }}" class="btn-custom danger" onclick="return confirm('Are You Sure To Delete?')">
...
<!-- Add Modal -->
<div class="modal fade text-left" id="Fabrications" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('fabrications.add')
                <form action="{{ route('admin.fabricationsAction', ['add']) }}" method="POST">
                    @csrf
...
<div class="modal fade text-left" id="Edit_{{ $d->id }}" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('fabrications.edit')
                <form action="{{ route('admin.fabricationsAction', ['update', $d->id]) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
