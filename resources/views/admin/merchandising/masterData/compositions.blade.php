...
@section('title')
<title>{{ websiteTitle('Compositions List') }}</title>
@endsection
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3>Compositions List</h3>
        <div class="dropdown">
            @can('compositions.add')
            <a href="javascript:void(0)" class="btn-custom primary" data-toggle="modal" data-target="#Compositions" style="padding:5px 15px;">
                <i class="bx bx-plus"></i> Add Composition
            </a>
            @endcan
            <a href="{{ route('admin.compositions') }}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.compositions') }}">
            <div class="row">
        ...
        <form method="GET" action="{{ route('admin.compositions') }}">
            <div class="row mb-2">
                <div class="col-md-4">
                    @if(can('compositions.edit') || can('compositions.delete'))
                ...
                <div class="col-md-4">
                    <ul class="statuslist">
                        <li><a href="{{ route('admin.compositions') }}">All ({{ $report->total }})</a></li>
                        <li><a href="{{ route('admin.compositions', ['status' => 'active']) }}">Active ({{ $report->active }})</a></li>
                        <li><a href="{{ route('admin.compositions', ['status' => 'inactive']) }}">Inactive ({{ $report->inactive }})</a></li>
                    </ul>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if(can('compositions.edit') || can('compositions.delete'))
                                    ...
                                    @endcan
                                    @can('compositions.delete')
                                    <a href="{{ route('admin.compositionsAction', ['delete', $d->id]) }}" class="btn-custom danger" onclick="return confirm('Are You Sure To Delete?')">
...
<!-- Add Modal -->
<div class="modal fade text-left" id="Compositions" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('compositions.add')
                <form action="{{ route('admin.compositionsAction', ['add']) }}" method="POST">
                    @csrf
...
<div class="modal fade text-left" id="Edit_{{ $d->id }}" tabindex="-1" data-backdrop="static" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @can('compositions.edit')
                <form action="{{ route('admin.compositionsAction', ['update', $d->id]) }}" method="POST">
                    @csrf
                    <div class="form-group mb-2">
                        <label>Name *</label>
